...
if daten.empty:
    ...
    # 4. KI-AUSWERTUNG GANZ AM ENDE
    st.subheader("🤖 KI-Gesamtbewertung (Gemini)")
    if text_fuer_ki:
        with st.spinner("Gemini liest und fasst die Nachrichten zusammen..."):
            analyse = ki_auswertung(auswahl, text_fuer_ki)
            # NEU: Wir nutzen st.markdown statt st.info, damit die Fettschrift 
            # und die Stichpunkte von der KI im Dashboard schön gerendert werden!
            st.markdown(analyse) 
    else:
        st.write("Warten auf Daten...")
